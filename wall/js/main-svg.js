...
Character.prototype = {
  ...
  updateTransform: function() {
    if (!this.bbox)
      this.bbox = this.ui.getBBox();
    var x_offset = this.bbox.x + this.bbox.width/2;
    var y_offset = this.bbox.y*2 + this.bbox.height;
    var x = document.documentElement.clientWidth/2 - x_offset;
    var y = document.documentElement.clientHeight - y_offset;
    var transform = "translate(" + [x,y].join(" ") + ") "+
                    "rotate(" + [this.angle,x_offset,y_offset].join(" ") + ")";
    this.ui.setAttribute("transform", transform);
  },
}
